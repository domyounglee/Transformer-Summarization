...
def _encode_and_add_eos(line, subtokenizer):
  ...
  if len(ids)>_ENCODE_LENGTH-1:
    ids = subtokenizer.encode(line)[:_ENCODE_LENGTH-1]
...
def translate_file(
  ...
  def input_fn():
    ...
    ds = tf.data.Dataset.from_generator(
        input_generator, tf.int64, tf.TensorShape([None]))#define thatsequence length
    ds = ds.padded_batch(batch_size, [_ENCODE_LENGTH],drop_remainder=True)#define the sequence length 
    print(ds)
    print("=============================")
    return ds
  ...
  for i, prediction in enumerate(estimator.predict(input_fn, hooks= hooks)):
    ...
    if print_all_translations:
      ...
      tf.logging.info(prediction["scores"])
...
def define_translate_flags():
  ...
  flags.DEFINE_string(
      name="underscored_ids", default="32371,32369,32366,32361,32353,32352,32349",
      help=flags_core.help_wrap(
          "choose more than 5 word ids which are basically underscored words to generate non trigrams"
          "make sure to pick those ids from the bottom of your vocabulary (unfrequently used ids)"
          "we are using it to implement trigram blocking in a tensorflow way"))
  flags.mark_flag_as_required("underscored_ids")
  flags.DEFINE_string(
