...
from official.transformer import translate_cls as translate
from official.transformer.model import model_params_cls as model_params
from official.transformer.model import transformer_cls as transformer #when training
#from official.transformer.model import transformer #when decoding
from official.transformer.utils import dataset_cls as dataset 
...
def model_fn(features, labels, mode, params):
  ...
  with tf.variable_scope("model"):
    ...
    logits.set_shape(targets["targets"].shape.as_list() + logits.shape.as_list()[2:])

    # Calculate model loss.
    # xentropy contains the cross entropy loss of every nonpadding token in the
    # targets.
    xentropy, weights = metrics.padded_cross_entropy_loss(
        logits, targets["targets"], params["label_smoothing"], params["vocab_size"])
    ...
    if mode == tf.estimator.ModeKeys.EVAL:
      summary_hook=tf.train.SummarySaverHook(save_steps=1000,output_dir=os.path.join(params["model_dir"],"vis"),summary_op=tf.summary.merge_all())
      ...
      return tf.estimator.EstimatorSpec(
          mode=mode, loss=loss, predictions={"predictions": logits},evaluation_hooks= [summary_hook],
          eval_metric_ops=metrics.get_eval_metrics(logits, labels["targets"], params))
    else:
      ...
      for key, value in metric_dict.items():
        ...
        tf.logging.info(key)
      summary_hook=tf.train.SummarySaverHook(save_steps=1000,output_dir=params["model_dir"],summary_op=tf.summary.merge_all())
...
def get_learning_rate(learning_rate, hidden_size, learning_rate_warmup_steps):
  ...
  with tf.name_scope("learning_rate"):
    ...
    ###domyoung 2019.10.1###
    """
    #noam learning rate scheme 
    learning_rate *= (hidden_size ** -0.5)
    #optimizer correction
    learning_rate *= 0.002
    #linear warm up 
    learning_rate *= 5000 * tf.minimum((step+1) * warmup_steps**-1.5, (step +1)**-0.5)
    #######
    """
    ###domyoung 2019.11.20 ###
    # like  "constant*linear_warmup*rsqrt_decay*rsqrt_hidden_size" in tensor2tensor
    learning_rate=tf.constant(2.0) #base learing rate constant is 2.0 
    learning_rate*=tf.minimum(1.0, step/warmup_steps) #linear_warmup
    learning_rate*=tf.rsqrt(tf.maximum(step, warmup_steps)) #rsqrt_decay
    learning_rate*= hidden_size ** -0.5
    #######
    """
    # Apply linear warmup
...
def define_transformer_flags():
  ...
  flags.DEFINE_string(
      ...
      help=flags_core.help_wrap(
          ...
          "the vocab file."))
  flags.DEFINE_integer(
      name="save_checkpoints_steps", short_name="scs", default=50000,
      help=flags_core.help_wrap(
          "the vocab file."))
...
def construct_estimator(flags_obj, params, schedule_manager):
  ...
  if not params["use_tpu"]:
    ...
    return tf.estimator.Estimator(
        model_fn=model_fn, model_dir=flags_obj.model_dir,
         params=params,
        config=tf.estimator.RunConfig(train_distribute=distribution_strategy,keep_checkpoint_max=3,
