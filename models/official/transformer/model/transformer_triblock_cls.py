...
from official.transformer.utils.tokenizer_cls import EOS_ID
...
class Transformer(object):
  ...
  def __call__(self, inputs, targets=None):
    ...
    with tf.variable_scope("Transformer", initializer=initializer):
      ...
      self.cls_attention_bias = model_utils.get_padding_bias(tf.cast(tf.equal(inputs,2),tf.int64))
      if targets is not None:
        self.cls_dec_attention_bias = model_utils.get_cls_dec_attention_bias(tf.cast(tf.equal(targets,2),tf.int64))
      else:
        self.cls_dec_attention_bias=None
      ...
      if targets is None:
        ...
        logits = self.decode(targets, encoder_outputs, attention_bias,cls_attention_bias=None, cls_dec_attention_bias=None,identity_mask=None)
        return logits
  ...
  def decode(self, targets, encoder_outputs, attention_bias,cls_attention_bias=None, cls_dec_attention_bias=None,identity_mask=None):
    ...
    with tf.name_scope("decode"):
      ...
      outputs = self.decoder_stack(
          ...
          attention_bias ,cls_attention_bias=cls_attention_bias, cls_dec_attention_bias=cls_dec_attention_bias,identity_mask=None)
  ...
  def _get_symbols_to_logits_fn(self, max_decode_length):
    ...
    def symbols_to_logits_fn(ids, i, cache):
      ...
      ### domyounglee 2020.2.12
      cls_dec_bias=model_utils.get_cls_dec_attention_bias(tf.cast(tf.equal(decoder_input,2),tf.int64))
      #self.cls_attention_bias=None
      ...
      decoder_outputs = self.decoder_stack(
          ...
          cache.get("encoder_decoder_attention_bias"), cls_attention_bias=None, cls_dec_attention_bias=None,identity_mask=None,cache=cache)
  ...
  def predict(self, encoder_outputs, encoder_decoder_attention_bias):
    ...
    ####domyoung 2019.12.9#####
    nontrigrams= nontrigram_generator(max_decode_length,self.params["underscored_ids"])
    ...
    hashTable = tf.contrib.lookup.HashTable(
      tf.contrib.lookup.KeyValueTensorInitializer(key,value), default_value)
