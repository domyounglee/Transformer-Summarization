...
class EmbeddingSharedWeights(tf.layers.Layer):
  ...
  def build(self, _):
    with tf.variable_scope("embedding_and_softmax", reuse=tf.AUTO_REUSE):
      ...
      self.seg = tf.get_variable(
          "seg_weights", [SEG, self.hidden_size],
          initializer=tf.random_normal_initializer(
              0., self.hidden_size ** -0.5))
  ...
  def call(self, x,mode):
    ...
    Args:
      x: Tuple of int64 tensors (input, input_seg, input_sent_mask) with shape [batch_size, length]
    ...
    with tf.name_scope("embedding"):
      ...
      if mode == "encode":
        mask = tf.to_float(tf.not_equal(x["inputs"], 0))

        embeddings = tf.gather(self.shared_weights, x["inputs"])
        ...
        """
        src_seg_embeddings = tf.gather(self.seg, x["input_seps"])

        src_seg_embeddings *= tf.expand_dims(mask, -1)
        src_seg_embeddings *= self.hidden_size ** 0.5


        embeddings += src_seg_embeddings
        """
        # 
        #          
      elif mode == "decode":
        mask = tf.to_float(tf.not_equal(x["targets"], 0))

        embeddings = tf.gather(self.shared_weights, x["targets"])
        ...
        """
        tgt_seg_embeddings = tf.gather(self.seg, x["target_seps"])

        tgt_seg_embeddings *= tf.expand_dims(mask, -1)
        tgt_seg_embeddings *= self.hidden_size ** 0.5


        embeddings += tgt_seg_embeddings
        """
