...
def inf(dtype):
  ...
  if dtype == "float32" or dtype == "bfloat16":
    return 1e7 #domyoung : just incase for the trigram score 
  elif dtype == "float16":
...
class SequenceBeamSearch(object):
  ...
  def __init__(self,
    ...
    self.nontrigrams = nontrigrams
    self.jj = 1
    self.tokens = None
    self.trigrams = None
    self.check_trigram = None
    ...
    if self.padded_decode:
      state_shape_invariants = {
          ...
          _StateKeys.ALIVE_CACHE:
              nest.map_structure(_get_shape_setzerodim_to_none, alive_cache),
  ...
  def _grow_alive_seq(self, state):
    ...
    #flat_ids is the current ids of every batch * beam
    if self.padded_decode:
      flat_ids = tf.reshape(
          tf.slice(alive_seq, [0, 0, i], [self.batch_size, self.beam_size, 1]),
          [self.batch_size * self.beam_size, -1])
    else:
      flat_ids = _flatten_beam_dim(alive_seq)  # [batch_size * beam_size, curidx+1]
    ...
    ###domyoung 12.10  starts trigram ###
    #_flat_ids is every predictd ids so far
    _flat_ids = tf.identity(_flatten_beam_dim(alive_seq))

    tokens = self.hashTable.lookup(_flat_ids)

    nontri_tokens=self.hashTable.lookup(self.nontrigrams)

    tokens = tf.concat([tokens,nontri_tokens],-1)

    
    tokens = tf.split(tf.reshape(tokens,(self.batch_size*self.beam_size,-1)), num_or_size_splits = tokens.shape[1], axis = -1) #tokens.shape[1]

    tokens = tf.strings.join(tokens)

    tokens = tf.strings.split(tokens,"_")

    tokens = tf.sparse.to_dense(tokens,default_value= tf.constant("",dtype=tf.string))
    tokens = tf.squeeze(tokens,1)

    tokens = tokens[:,:self.max_decode_length+1]
    self.tokens = tf.reshape(tokens, [self.batch_size*self.beam_size,self.max_decode_length+1],name="Tokens")

    self.check_trigram = tf.constant(np.array([False]*self.batch_size*self.beam_size,dtype=bool),dtype=tf.bool,name="check")
    
    self.trigrams =  tf.strings.join([self.tokens[:,0],self.tokens[:,1],self.tokens[:,2]])
    self.trigrams = tf.expand_dims(self.trigrams, -1)

    self.j= tf.constant(1,dtype=tf.int32)
  

    
    trigrams_,_,self.check_trigram,_ = tf.while_loop(
      self.cond,
      self.body,
      [self.trigrams, self.j, self.check_trigram,self.tokens],
      shape_invariants= \
                     [tf.TensorShape([self.batch_size*self.beam_size,None]),
                     tf.TensorShape([]),tf.TensorShape([self.batch_size*self.beam_size,]),
                     tf.TensorShape([self.batch_size*self.beam_size,self.max_decode_length+1])],
                     parallel_iterations=1, 
                     back_prop=False)

  
    
    trigram_score= tf.cast(self.check_trigram, self.dtype) * -inf(self.dtype)
    #trigram_score= tf.cast(self.check_trigram, self.dtype) * -1e8
    trigram_score= _unflatten_beam_dim(trigram_score,self.batch_size,self.beam_size)
    if self.use_trigram:
      log_probs += tf.expand_dims(trigram_score,axis=2)
  ...
  def cond(self,trigrams,j,check_trigram,tokens):
    return j<tf.shape(self.tokens)[-1]-2
    

  def body(self,trigrams, j , check_trigram,tokens):

    trigrams = tf.reshape(trigrams, [self.batch_size*self.beam_size,-1])
    
    
    trigram = tf.strings.join([tokens[:,j],tokens[:,j+1],tokens[:,j+2]])
    trigram = tf.expand_dims(trigram,-1)

    
    tile_dims = [1] * trigram.shape.ndims
    tile_dims[-1] = j

    tiled_trigram = tf.tile(trigram, tile_dims)
    
    equal = tf.math.equal(trigrams,tiled_trigram)

    reduce_equal = tf.reduce_any(equal, -1)
    check_trigram = tf.math.logical_or(reduce_equal, check_trigram)
    trigrams = tf.concat([trigrams,trigram],-1)

    return (trigrams, tf.add(j,1), check_trigram,tokens)
...
def sequence_beam_search(
    symbols_to_logits_fn, initial_ids, initial_cache, vocab_size, hashTable, nontrigrams, beam_size,batch_size,
    alpha, max_decode_length, use_trigram,eos_id, padded_decode=True):
  ...
  sbs = SequenceBeamSearch(symbols_to_logits_fn, vocab_size,hashTable, nontrigrams, batch_size,
                           beam_size, alpha, max_decode_length, eos_id,
                           padded_decode,use_trigram=use_trigram)
...
def _shape_list(tensor):
  ...
  for i in range(len(shape)):  # pylint: disable=consider-using-enumerate
    if shape[i] is None and i ==0 :
      shape[i] = 10 #batch_size 
    if shape[i] is None and i == len(shape)-1:
      shape[i] = 512 #inputlength or hidden size , now it is the same 
...
def _get_shape_setzerodim_to_none(tensor):
  shape_list = _shape_list(tensor)
