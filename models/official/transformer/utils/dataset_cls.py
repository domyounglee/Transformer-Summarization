...
def _parse_example(serialized_example):
  ...
  data_fields = {
      ...
      "input_seps": tf.VarLenFeature(tf.int64), 
      "input_cls_mask": tf.VarLenFeature(tf.int64), 
      "targets": tf.VarLenFeature(tf.int64), 
      "target_seps": tf.VarLenFeature(tf.int64), 
      "target_cls_mask": tf.VarLenFeature(tf.int64)
  }
  ...
  input_seps = tf.sparse_tensor_to_dense(parsed["input_seps"])
  input_cls_mask = tf.sparse_tensor_to_dense(parsed["input_cls_mask"])
  targets = tf.sparse_tensor_to_dense(parsed["targets"])
  target_seps = tf.sparse_tensor_to_dense(parsed["target_seps"])
  target_cls_mask = tf.sparse_tensor_to_dense(parsed["target_cls_mask"])
  
  src={}
  tgt={}
  src["inputs"]=inputs
  src["input_seps"]=input_seps
  src["input_cls_mask"]=input_cls_mask
  tgt["targets"]=targets
  tgt["target_seps"]=target_seps
  tgt["target_cls_mask"]=target_cls_mask
...
def _filter_max_length(example, max_length=256):
  ...
  return tf.size(example["inputs"]) <= max_length 
  #return tf.logical_and(tf.size(example[0]) <= max_length,
  #                      tf.size(example[1]) <= max_length)
def _truncate_length(x,y,input_length,output_length):
  for k,v in x.items():
    x[k]=v[:input_length] 
  for k,v in y.items():
    y[k]=v[:output_length] 
...
def _batch_examples(dataset, batch_size, max_length):
  ...
  def example_to_bucket_id(example_input, example_target):
    ...
    seq_length = _get_example_length((example_input["inputs"], example_target["targets"]))
  ...
  def batching_fn(bucket_id, grouped_dataset):
    ...
    return grouped_dataset.padded_batch(bucket_batch_size, ({"inputs":[None],"input_seps":[None],"input_cls_mask": [None]},\
                                                            {"targets":[None],"target_seps":[None],"target_cls_mask":[None]}))
...
def _read_and_batch_from_files(
    file_pattern, batch_size, max_length,max_input_length, max_output_length,  num_parallel_calls, shuffle, repeat,
    static_batch=False):
  ...
  dataset = dataset.map(lambda x, y : _truncate_length(x,y,max_input_length,max_output_length))
  ########
...
def train_input_fn(params):
  ...
  file_pattern = os.path.join(params["data_dir"] or "", "train.tfrecord")
  if params["use_synthetic_data"]:
    return _generate_synthetic_data(params)
  return _read_and_batch_from_files(
      file_pattern, params["batch_size"], params["max_length"],params["max_input_length"],
      params["max_output_length"], params["num_parallel_calls"], shuffle=True,
      repeat=params["repeat_dataset"], static_batch=params["static_batch"])


def eval_input_fn(params):
  """Load and return dataset of batched examples for use during evaluation."""
  file_pattern = os.path.join(params["data_dir"] or "", "valid.tfrecord")
  if params["use_synthetic_data"]:
    return _generate_synthetic_data(params)
  return _read_and_batch_from_files(
      file_pattern, params["batch_size"],  params["max_length"],params["max_input_length"],
      params["max_output_length"], params["num_parallel_calls"], shuffle=False, repeat=1,
